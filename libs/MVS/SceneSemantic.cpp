...
* SceneTexture.cpp
*
* Copyright (c) 2019-2020 Freeverc 
*
* Author(s):
*
*      Freeverc <freeverc@gmail.com>
*
*/
...
public:
	MeshSemantic(Scene& _scene, unsigned _nResolutionLevel=0, unsigned _nMinResolution=640);
	~MeshSemantic();
	...
	void GenerateSemantic(bool bGlobalSeamLeveling, bool bLocalSeamLeveling, unsigned nTextureSizeMultiple, unsigned nRectPackingHeuristic, Pixel8U colEmpty);
...
MeshSemantic::MeshSemantic(Scene& _scene, unsigned _nResolutionLevel, unsigned _nMinResolution)
	:
...
void MeshSemantic::ListVertexFaces()
{
...
bool MeshSemantic::ListCameraFaces(FaceDataViewArr& facesDatas, float fOutlierThreshold)
{
...
bool MeshSemantic::FaceOutlierDetection(FaceDataArr& faceDatas, float thOutlier) const
{
...
bool MeshSemantic::FaceViewSelection(float fOutlierThreshold, float fRatioDataSmoothness)
{
	// extract array of triangles incident to each vertex
	ListVertexFaces();
...
void MeshSemantic::CreateSeamVertices()
{
...
void MeshSemantic::GlobalSeamLeveling()
{
...
void MeshSemantic::ProcessMask(Image8U& mask, int stripWidth)
{
...
inline MeshSemantic::Color ColorLaplacian(const Image32F3& img, int i) {
	const int width(img.width());
	return img(i-width) + img(i-1) + img(i+1) + img(i+width) - img(i)*4.f;
}

void MeshSemantic::PoissonBlending(const Image32F3& src, Image32F3& dst, const Image8U& mask, float bias)
{
...
void MeshSemantic::LocalSeamLeveling()
{
	ASSERT(!seamVertices.IsEmpty());
	const unsigned numPatches(texturePatches.GetSize()-1);
...
void MeshSemantic::GenerateSemantic(bool bGlobalSeamLeveling, bool bLocalSeamLeveling, unsigned nTextureSizeMultiple, unsigned nRectPackingHeuristic, Pixel8U colEmpty)
{
...
// semantic mesh
bool Scene::SemanticMesh(unsigned nResolutionLevel, unsigned nMinResolution, float fOutlierThreshold, float fRatioDataSmoothness, bool bGlobalSeamLeveling, bool bLocalSeamLeveling, unsigned nTextureSizeMultiple, unsigned nRectPackingHeuristic, Pixel8U colEmpty)
{
	MeshSemantic semantic(*this, nResolutionLevel, nMinResolution);

	// assign the best view to each face
	{
		TD_TIMER_STARTD();
		if (!semantic.FaceViewSelection(fOutlierThreshold, fRatioDataSmoothness))
			return false;
	...
	{
		TD_TIMER_STARTD();
		semantic.GenerateSemantic(bGlobalSeamLeveling, bLocalSeamLeveling, nTextureSizeMultiple, nRectPackingHeuristic, colEmpty);
		DEBUG_EXTRA("Generating texture atlas and image completed: %u patches, %u image size (%s)", semantic.texturePatches.GetSize(), mesh.textureDiffuse.width(), TD_TIMER_GET_FMT().c_str());
	}
